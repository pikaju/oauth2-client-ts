...
import { ResourceOwnerPasswordCredentialsGrant } from '../../lib/grant';

describe('Flow: Resource Owner Password Credentials Grant', () => {
  ...
  it('returns the correct token credentials object', async () => {
    const handle = client.startResourceOwnerPasswordCredentialsFlow();
    const token = await handle.getToken(new ResourceOwnerPasswordCredentialsGrant('jsc', 'passw0rd'), 'email.read');
  ...
  it('throws client errors', async () => {
    const handle = client.startResourceOwnerPasswordCredentialsFlow();
    await expect(handle.getToken(new ResourceOwnerPasswordCredentialsGrant('doesnt', 'exist'))).to.eventually.be.rejectedWith(InvalidGrantError);
  });

  it('throws networking errors', async () => {
    const handle = client.startResourceOwnerPasswordCredentialsFlow();
    await expect(handle.getToken(new ResourceOwnerPasswordCredentialsGrant('ako', 'sgehdn'))).to.eventually.be.rejectedWith(NetworkingError);
  });
});
